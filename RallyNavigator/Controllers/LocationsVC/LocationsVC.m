...
- (void)viewDidLoad
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_isFirstTime)
        {
            [_mapBoxView setZoomLevel:16 animated:NO];
        }
        else
        {
            [self btnTogglePreferrenceClicked:nil];
...
- (void)callWSToUpdate
{
    if (_strRouteIdentifier == nil)
    {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            _btnAdd.enabled = !_isFirstTime;
            _btnRecording.enabled = !_isFirstTime;
            _btnText.enabled = !_isFirstTime;
            _btnCamera.enabled = !_isFirstTime;
        });
        return;
    }
...
- (void)playAudio:(NSString *)strFileName :(NSString *)strFileType
{
    ...
    if (_isFirstTime && arrMapBoxMarkers.count == 0 && !isPaused)
    {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (arrMapBoxMarkers.count == 0)
            {
                ...
                if (isAutoPhotoEnabled && imgCaptured == nil && _sessionRunning)
                {
                    [self capturePhoto];
                }
                else if (!isAutoPhotoEnabled)
                {
                    [self playAudio:@"beep1_02" :@"mp3"];
                }
...
- (IBAction)btnRecordingClicked:(id)sender
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self playAudio:@"TextWithRecord" :@"wav"];
...
- (void)flashOff:(UIView *)v isEditable:(NSString *)strIsEditable button : (UIButton*)btnFlash
{
    [UIView animateWithDuration:0.6 delay:0 options:UIViewAnimationOptionAllowUserInteraction animations:^ {
        ...
        else
        {
         ...
         if (!isCancelled && isRecordingStarted) {
...
{
    [UIView animateWithDuration:0.7 delay:0 options:UIViewAnimationOptionAllowUserInteraction animations:^ {
        if ([strIsEditable isEqualToString:@"YES"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            {
                if (sender != nil)
                {
                    ...
                    if (isEditEnabled)
                    {
                        NSIndexPath *idPath = [_tblLocations indexPathForCell:cell];
                    ...
                    if (((UIButton *)sender).tag != -1)
                    {
                        ...
                        if (isEditEnabled)
                        {
                            [_tblLocations setScrollEnabled:NO];
                            strEditWayPointDescription = cell.txtView.text;
                            [self manageCell:cell forIndexPath:[_tblLocations indexPathForCell:cell]];
                        }
                        else
                        {
                            NSPredicate *predicate = [NSPredicate predicateWithBlock:^BOOL(Locations *objLocation, NSDictionary<NSString *,id> * _Nullable bindings) {
                            ...
                            { 
                                ...
                                Locations *objLocation = [arrSearchResults firstObject];
                                objLocation.text = strEditWayPointDescription;
                                if (imgEditCaptured != nil)
                                {
                                    objLocation.photos = @[imgEditCaptured];
                                }
                                
                                if (audioEditData != nil)
                                {
                                    objLocation.audios = @[audioEditData];
                                }
                                
                                [arrAllLocations replaceObjectAtIndex:index withObject:objLocation];
                                
                                NSIndexPath *indexPath = [_tblLocations indexPathForCell:cell];
                                [_tblLocations reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
                            }
                            
                            [self playAudio:@"beep1_02" :@"mp3"];
                            NSString *strWayPointId = [NSString stringWithFormat:@"%ld", ((UIButton *)sender).tag];
                            [self saveDataForUpdatingWayPoint:strWayPointId];
                            [_tblLocations setScrollEnabled:YES];
                            audioEditData = nil;
                            imgEditCaptured = nil;
                ...
                for (NSInteger i = 0; i < arrAllLocations.count; i++)
                {
                    Locations *objLocation = [arrAllLocations objectAtIndex:i];
                    ...
                    if ([location distanceFromLocation:currentLocation] < /*objConfig.accuracy.minDistanceTrackpoint*/50)
                    {
                ...
                [arrAllLocations removeObjectsInArray:arrIndexes];
                
                counter = arrAllLocations.count;
                
                arrIndexes = [[NSMutableArray alloc] init];
                
                for (NSInteger i = arrRemainingTracks.count - 1; i >= 0; i--)
                {
                    NSDictionary *dic = [arrRemainingTracks objectAtIndex:i];
                    ...
                    if ([location1 distanceFromLocation:currentLocation] < /*objConfig.accuracy.minDistanceTrackpoint*/50)
                    {
                ...
                [arrRemainingTracks removeObjectsInArray:arrIndexes];
    ...
    dispatch_async(self.sessionQueue, ^{
        switch (self.setupResult)
        {
            case AVCamSetupResultSuccess:
            {
                ...
                if (_isFirstTime && arrMapBoxMarkers.count == 1 && isAutoPhotoEnabled && imgCaptured == nil)
                {
                                                    ...
                                                    willCapturePhotoAnimation:^{}
                                                      livePhotoCaptureHandler:^(BOOL capturing) {
                                                          ...
                                                          dispatch_async(self.sessionQueue, ^{
                                                              ...
                                                              _inProgressPhotoCaptureDelegates = [[NSMutableDictionary alloc] init];
                                                          });
                                                      }];
...
- (NSString *)manageForRouteId:(NSString *)strRouteId
{
    ...
    if (arrGeoLocations.count > 0)
    {
        ...
        dispatch_async(backgroundQueue, ^(void) {
            ...
            MGLPolylineFeature *temp = o_polylineMapBox;
            o_polylineMapBox = (MGLPolylineFeature *)shapeCollectionFeature.shapes.firstObject;
            
            o_polylineMapBox.title = polylineMapBox.attributes[@"Test Name1"]; // "Crema to Council Crest"
            
            __weak typeof(self) weakSelf = self;
            dispatch_async(dispatch_get_main_queue(), ^(void) {
                [weakSelf.mapBoxView addAnnotation:o_polylineMapBox];
        ...
        [self presentConfirmationAlertWithTitle:@"Download Offline Maps Around Overlay Track?"
                          ...
                          withCancelButtonTitle:@"No"
                             ...
                             withExecutionBlock:^{
                                 ...
                                 vc.strMapName = overlayName;
                                 vc.delegate = self;
                                 
                                 vc.overlaySender = @{
                                                      @"markers" : arrMapBoxMarkers1,
                                                      @"polyline" : o_polylineMapBox
                                                      };
