...
- (void)viewDidLoad
{
    ...
    [self pullToRefreshHeaderSetUpForTableView:_tblOfflineMaps
                                    withStatus:@"Get new maps"
                           withRefreshingBlock:^{
                               if ([self isHeaderRefreshingForTableView:self.tblOfflineMaps]) {
                                   [self.tblOfflineMaps.mj_header endRefreshing];
                               }
...
- (void)deleteOfflineMap:(MGLOfflinePack*)offlinePack
{
    ...
    [[MGLOfflineStorage sharedOfflineStorage] removePack:offlinePack
                                   withCompletionHandler:^(NSError* _Nullable error) {
                                       ...
                                       if (!error) {
                                           NSUInteger index = [self->arrPacks indexOfObject:offlinePack];
                                           NSIndexPath* indexPath = [NSIndexPath indexPathForRow:index inSection:0];
                                           [self->arrPacks removeObjectAtIndex:indexPath.row];
                                           [self.tblOfflineMaps beginUpdates];
                                           [self.tblOfflineMaps deleteRowsAtIndexPaths:@[ indexPath ] withRowAnimation:UITableViewRowAnimationLeft];
                                           [self.tblOfflineMaps endUpdates];
                                           [self manageProgress];
                                       } else {
...
- (void)offlinePackProgressDidChange:(NSNotification*)notification
{
    ...
    if ([arrPacks containsObject:pack]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.tblOfflineMaps insertRowsAtIndexPaths:@[ [NSIndexPath indexPathForRow:0 inSection:0] ] withRowAnimation:UITableViewRowAnimationNone];
        });
    }
