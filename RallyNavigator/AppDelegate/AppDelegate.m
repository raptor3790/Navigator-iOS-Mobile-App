...
- (void)processForData
{
    if (objSyncData.imageData.length > 0)
    {
        [self saveImageOverServer:^(BOOL success) {
            if (success)
            {
                if (objSyncData.voiceData.length > 0)
                {
...
//                NSLog(@"Upload failed: [%@]", task.error);
            ...
            _normalImgCompletionHandler(NO);
            _normalImgCompletionHandler = nil;
        }
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                    ...
                    NSString *strJsonData = objSyncData.jsonData;
...
//                        NSLog(@"DICTIONARY");
                        ...
                        objSyncData.jsonData = strJsonData;
                        [CoreDataHelper save];
                    }
...
//                        NSLog(@"ARRAY");
                    ...
                    _normalImgCompletionHandler(YES);
                    _normalImgCompletionHandler = nil;
                }];
            });
        }
...
//                NSLog(@"Upload failed: [%@]", task.error);
            ...
            _normalAudioCompletionHandler(NO);
            _normalAudioCompletionHandler = nil;
        }
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
...
//                        NSLog(@"ARRAY");
                    ...
                    _normalAudioCompletionHandler(YES);
                    _normalAudioCompletionHandler = nil;
                }];
            });
        }
...
//                NSLog(@"ARRAY");
        ...
        NSData *data = [objSyncData.jsonData dataUsingEncoding:NSUTF8StringEncoding];
        ...
        [[WebServiceConnector alloc] init:strURL
                           ...
                           forServiceType:(ServiceType)[objSyncData.serviceType integerValue]
                           showDisplayMsg:@""
                               showLoader:NO];
    });
}
...
- (IBAction)handleWayPointResponse:(id)sender
{
    ...
    if ([dic valueForKey:RouteKey] && [[dic valueForKey:SUCCESS_STATUS] boolValue])
    {
        objSyncData.isActive = [NSNumber numberWithInteger:1];
        [CoreDataHelper save];
        
        if ([objSyncData.routeIdentifier doubleValue] < 0)
        {
            NSString *strOldRouteId = [NSString stringWithFormat:@"%ld", (long)[objSyncData.routeIdentifier doubleValue]];
            ...
            if (arrResponse.count > 0)
            {
                ...
                NSArray *arrCDUser = [[[CDSyncData query] where:[NSPredicate predicateWithFormat:[NSString stringWithFormat:@"routeIdentifier='%ld'",(long)[objSyncData.routeIdentifier doubleValue]]]] all];
        ...
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                    ...
                    [_arrEditData addObject:dicAdd1];
                    [_arrEditData addObject:dicAdd2];
                    
                    _imgCompletionHandler(YES);
                    _imgCompletionHandler = nil;
                }];
            });
        }
        return nil;
        ...
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                    ...
                    _audioCompletionHandler(YES);
                    _audioCompletionHandler = nil;
                }];
            });
        }
        return nil;
